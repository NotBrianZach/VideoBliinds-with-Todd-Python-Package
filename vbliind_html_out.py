...
#brisqueNames = [f for f in os.listdir('~/vbliindTrainingOutput') if re.match(r'.*BRISQUE.*', f)]
##print "brisqueNames"
##print brisqueNames
#brisqueCounter = 0
...
#vTotalNames = [f for f in os.listdir('~/vbliindTrainingOutput') if re.match(r'vscore*', f)]
#vTotalScores = []
#vTotalCounter = 0
...
vTotalCounter = 0
vScoreComponents = []
path = os.path.expanduser('~/vbliindTrainingOutput/')
vComponentFiles = [f for f in os.listdir(path) if re.match(r'features*', f)]
for name in vComponentFiles:
    open_file = open(path + name, 'r')
    for line in open_file:
        # 'line' is a line in your file
        # We make sure that 'line' is non-empty
        if line:
            vScoreComponents+=line.split()
            vTotalCounter+=1
    # When finishing reading the file, we close it    
    open_file.close()
...
#for vScoreComponents in allComponentScores:
...
#spatial naturalness = niqe
#nvs-shape parameter ratios = geo_ratio_features, cohenerncy measure = mean_coh10x10
#global motion measure = G
# Print the content of the table, line by line ----
print "<tr><td>Name</td><td>Spatial Naturalness</td><td>DC Feature 1</td><td>DC Feature 2</td><td>NVS Shape-Parameter ratios</td><td>Coherency Measure</td><td>Global Motion Measure</td></tr>"
for i in range(0, vTotalCounter):
        print "<tr><td>"+vComponentFiles[i]+"</td><td>"+spatialNaturalness[i] + "</td><td>" + vScoreComponents[i*15+4] + "</td><td>"+vScoreComponents[i*15+6]+"</td><td>" + shapeParameterRatios[i] + "</td><td>"+ vScoreComponents[i*15+11]+"</td><td>"+vScoreComponents[i*15+13]+"</td></tr>"
