...
%     x
    ...
    for i = 1 : mbsize : row-mbsize+1
        for j = 1 : mbsize : col-mbsize+1
            dct_motion_comp_diff(i:i+mbsize-1,j:j+mbsize-1,x) = dct2(frames(i:i+mbsize-1, ...
                j:j+mbsize-1,x+1)-frames(i+motion_vects16x16(1,mbCount,x):...
                i+mbsize-1+motion_vects16x16(1,mbCount,x), ...
                j+motion_vects16x16(2,mbCount,x):j+mbsize-1+motion_vects16x16(2,mbCount,x),x));
...
%              frames(i:i+mbsize-1, ...
%                  j:j+mbsize-1,x+1)
%              frames(i+motion_vects16x16(1,mbCount,x):...
%                  i+mbsize-1+motion_vects16x16(1,mbCount,x), ...
%                  j+motion_vects16x16(2,mbCount,x):j+mbsize-1+motion_vects16x16(2,mbCount,x),x)
            mbCount = mbCount+1;
        end
...
%save('~/Desktop/VideoBliinds/matlabvects','dct_motion_comp_diff');
